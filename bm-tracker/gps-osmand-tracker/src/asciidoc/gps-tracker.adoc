...
The `gps-osm-tracker` is a Java Client Software to simulate 
...
and a communication unit. The GPS receives the location via satellite and
...
longitude, which the Server can use to localize the device.
...
the location on Earth - a highly mathematical number cruncher.
