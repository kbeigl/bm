...
a standard _hardware_ device that _collects_ GPS locations, optionally buffer 
...
In _Chapter 8 The JeeTS Tracker_ you can find a more complex implementation +
with a lot of background info on Server processing. +
Find an implementation in the 
https://github.com/kbeigl/jeets/tree/master/jeets-clients/jeets-device[JeeTS repo] +
This section describes a simplified Tracker. 
====
...
image::./diagrams/jeets-gps-tracker.png[JeeTS Tracker]

[quote,GPS Tracking with Java EE Components, Figure 8.1]   
A tracking hardware combines a GPS- and GSM unit on a Controller Board. 
The Controller evaluates sensor data to create event messages in the protocol format. 
NMEA data is constantly validated to provide the best position available. 
The messages can be sent whenever appropriate.
