...
We will use the Traccar GTS, which is build on the following Entities with fixed Relations:
...
Let's compare the generate `Device` class,
a `json devices` message sent via websocket +
and the attributes in the database model:

[source,java]
----
      class Device {             {devices=[        database table: TC_DEVICES
        id: 638                     {id=638,                       ID
        name: runner                 name=runner,                  NAME
        uniqueId: 10                 uniqueId=10,                  UNIQUEID
        status: offline              status=online,                STATUS
        disabled: false              disabled=false,               DISABLED
        lastUpdate: null             lastUpdate=2026-02-20...,     LASTUPDATE
        positionId: 0                positionId=0,                 POSITIONID
        groupId: 0                   groupId=0,                    GROUPID
        phone: null                  phone=null,                   PHONE
        model: ro                    model=ro,                     MODEL
        contact: null                contact=null,                 CONTACT
        category: null               category=null,                CATEGORY
        attributes: {}               attributes={},                ATTRIBUTES
                                     -------------                 ----------
                        relation:    calendarId=0,                 CALENDARID
                                     expirationTime=null           EXPIRATIONTIME
                                    }
                                 ]}             server attributes  MOTIONSTATE
                                                                   MOTIONTIME
                                                                   MOTIONDISTANCE
                                                                   OVERSPEEDSTATE
                                                                   OVERSPEEDTIME
...
All messages have the devices `uniqueId` in common to refer to the same device. +
Yet the models deviate by the two attributes `calendarId` and `expirationTime`. +
Important information for ETL stuff like mapping.

The Traccar Specification is defined by the Traccar Data Entities (Device, Position, Event, etc.)
is the Java source code. +
The websocket API simply takes these internal Java objects and converts them to JSON.
