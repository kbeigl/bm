...
- [x]  create a Springboot application to simulate a GPS Tracker hardware
- [x]  use the osmand protocol with Traccar ID
- [x]  use the OpenTools library for GPS coordinates and distance, bearing etc.
- [x]  setters for location and fixtime 
- [x]  send messages via Camel Route
- [x]  store messages in a thread-safe queue, if server is unavailable, send later
...
https://camel.apache.org/components/seda-component.html[SEDA component] `"seda:"`
out of the box

[quote, camel.apache.org/components/seda-component.html]
...
Therefore you can proceed to  
xref:../../bm-traccar/traccar-realtime-client/integrated-dev-cycle.adoc[IT roundtrip development]
to follow the development of Tracker Features.

== Traccar Server Events

The Traccar API supports sending GPS tracking data via the http OsmAnd protocol.
When tracking data is sent to this endpoint the traccar engine triggers
several handlers to register the data into the system and triggers
the realtime websocket connection to send out updates.
