...
The `traccar-api-client` is a Java Client Software to _request_ real time data 
...
Therefor this *WebSocket Route/Session Manager* can create a Camel channel 
for each individual registered user to manage WebSocket connections and event routing.
...
link:../../main/java/bm/traccar/ws/TraccarWsClientRoute.java[TraccarWsClientRoute]. +
