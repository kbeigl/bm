...
also known as StateManager in RealTime context.
    ...
    •  It uses `ConcurrentHashMap` for its internal state, 
       which provides thread-safe access and updates
...
[source,java]
----
class RealTimeManagerIT extends BaseRealTimeTest { // sets up scenario
...
The `addOrUpdatePosition` method shows how the `RealTimeManager` works.
It is called when a new position update is received, either from the WebSocket or any other
source. 
The method checks if the position already exists in the `positions` map. 
If it does, it updates the existing position; if not, it adds a new entry.
                    
This approach ensures that we maintain a consistent state in the `RealTimeManager`
while allowing for efficient updates without needing to replace entire objects in the map.
                    
Another vital aspect of programming are object references!

When receiving a new Device update, we could simply replace the whole Device object in the `devices` map.
However, this would break the reference to the `Device` by the users of the `RealTimeManager`. 
Copy-on-write (create a new merged Device and atomically replace it in the map) ensures 
that all references remain valid and consistent across the application.
Updating fields in-place preserves references, but requires explicit 
synchronization or thread-safe fields to avoid races and visibility problems.

We are using Spring's `BeanUtils` to copy the properties from the new Device to the existing one in the map.
...
Now that we can safely read from the `RealTimeManager` we will create a `RealTimeController` for synchronization. +
...
<3> Instantiated by the Spring framework. 
    Injects the RealTimeManager dependency via constructor injection.
...
These empty messages are useful as a connection heart beat and to detect connectivity issues.
...
and is
xref:websocket.adoc[described in more detail here].

=== sample messages

To keep a distributed system integrity you need to have all models in mind.

CHECK

xref:../../bm-tracker/gps-osmand-tracker/gps-tracker-impl.adoc#_the_model[The Model for Tracker Development]
defines a GPS message from a unique device. +
The REST `openapi.yml` file provides a bigger 
xref:../../bm-tracker/gps-osmand-tracker/gps-tracker-impl.adoc#_model_deviations[model for a complete GTS]. +
...
vice versa the `position` has the reference to `deviceId=554` - and *not* to the `uniqueId=10`!
