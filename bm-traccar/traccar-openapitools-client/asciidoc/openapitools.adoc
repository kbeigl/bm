...
*After generating* the `traccar-api-generated-x.y.z.jar` you can check the  

    target/generated-sources/openapi/README.md
    
in the `/target/generated-sources/openapi` folder +
for the API version and the generated API documentation. +
Also counter check the 

    target/generated-sources/openapi/api/openapi.yaml specification file

that was used for the generation +
and is stored in the `api/` folder of the generated project.
...
Then you can switch off the `traccar-openapitools-client` project in the parent POM with

            <openapi.skip>true</openapi.skip>
    
to save time and resources for the next builds, 
since the generation is only needed once per traccar release.
...
After running `mvn install` you can find the generated documents in
    
    target/generated-sources/openapi/docs
...
We are not interested in traccar server development directly,
since we abstract the server with its REST API.

In the `traccar-openapitools-client` project we are accessing the _versioned_ `openapi.yaml` from 
...
- [x] The creation of the `traccar-api-generated-x.y.z.jar` is completly independant +
...
file. If you diff this file against the one downloaded you might be surprised.
The file in the target is hundreds of lines longer with hundreds of differences.
Do not be alarmed.

The new file is not a direct download of the Traccar spec; it’s a processed and resolved version of it. +
The openapi-generator-maven-plugin parses, validates, and then rewrites it 
so that the internal generator engine has a clean, standardized map to build your code from.

If the original Traccar `openapi.yaml` uses `$ref` to point to other files, the generator resolves all of those links. 
It pulls all that external data into a single, massive, flat file. 
The generator often reorders keys to follow a strict internal logic. 
